...
When `FORWARD_URL` is set (e.g. `FORWARD_URL=http://asset-proxy:8080`), the proxy runs as a lightweight cache layer in front of a processing instance. `CACHE_BUCKET` must also be set in this mode. Both `http://` (h2c) and `https://` (h2 over TLS) are supported.
