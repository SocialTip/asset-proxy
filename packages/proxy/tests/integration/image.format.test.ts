...
describe("best format", () => {
  // test-image.png has entropy ~2.0 (low complexity → lossless WebP)
  // test-image-butterfly.png has entropy ~7.5 (high complexity → lossy WebP)
  ...
  it("selects lossless WebP for a low-complexity image", async () => {
  ...
  it("selects lossy WebP for a high-complexity image", async () => {
    const parsed = parseProcessingUrl(
  ...
  it("best format produces smaller output than PNG for a complex image", async () => {
    ...
    expect(bestBuf.length).toBeLessThan(pngBuf.length);
  });

  it("falls back to lossy WebP when image exceeds max resolution", async () => {
    ...
    // At w:200, output is ~200x150 = 30000 pixels which exceeds 0.005 MP → lossy WebP fallback.
    const parsed = parseProcessingUrl(
